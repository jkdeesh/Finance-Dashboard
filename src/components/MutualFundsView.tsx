...
import { MutualFund, CURRENCIES, CurrencyCode, convertCurrency } from '../types';
  ...
  return (
    <div className="space-y-6" id="mutual-funds-view-root">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <GlassCard className="flex items-center gap-4 col-span-1 md:col-span-2">
                ...
                >
                  <div className="space-y-3">
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <span className="text-indigo-950 dark:text-indigo-200 font-extrabold text-sm block group-hover:text-indigo-900 dark:group-hover:text-indigo-300">{fund.fundName}</span>
                        <span className="px-2 py-0.5 rounded-full bg-slate-100/80 dark:bg-slate-800/85 text-slate-600 dark:text-slate-300 border border-slate-200/50 dark:border-slate-700/50 text-[9px] font-bold mt-1.5 inline-block">
                          {fund.category}
                        </span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 py-2 border-y border-slate-200/10 text-xs font-mono">
                      <div>
                        ...
                        <span className="text-slate-500 dark:text-slate-400 text-[9px] block font-sans mt-0.5">{fund.units} units @ {formatAccountCurrency(fund.averageNav, fund.currency)}</span>
                      </div>
                      <div>
                        <span className="text-slate-500 dark:text-slate-300 text-[10px] block uppercase font-sans font-semibold">Market Value</span>
                        <div className="text-indigo-950 dark:text-indigo-100 font-extrabold">
                        ...
                        <span className="text-slate-500 dark:text-slate-400 text-[9px] block font-sans mt-0.5">Current NAV: {formatAccountCurrency(fund.currentNav, fund.currency)}</span>
                      </div>
                    </div>
                  </div>
